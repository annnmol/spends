...
const DEFAULT_ACCOUNTS: CreateAccountInput[] = [
  ...
  },
  {
    name: "Google Pay",
    type: "upi",
    slugs: ["gpay", "googlepay", "upi", "googlepay"],
    color: "#4285F4",
    last4digits: "9876",
  }
];
...
export function matchAccount(accounts: Account[], body: string): number | null {
  ...
  const last4Match = accounts.find(
    (a) => a.last4digits && upper.includes(a.last4digits),
  );
...
export async function unlinkTransactionsForAccount(
  accountId: number,
): Promise<void> {
  await dbRawExecute(
...
export async function linkAllUnlinkedTransactions(
  accounts: Account[],
): Promise<void> {
  ...
  for (const row of rows) {
    ...
    if (accountId !== null) {
      await dbRawExecute("UPDATE transactions SET accountId = ? WHERE id = ?", [
        accountId,
        row.id,
      ]);
    }
